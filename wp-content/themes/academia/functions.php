...
function academia_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'academia' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'academia_custom_background_args',
			array(
...
function academia_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'academia' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'academia' ),
...
function academia_scripts() {
	wp_enqueue_style( 'academia-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'academia-style', 'rtl', 'replace' );

	wp_enqueue_script( 'academia-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
